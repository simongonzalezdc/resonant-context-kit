...
set -e
REPO="${1:-$HOME/workspaces/augmentatism-recon/repos/2.0.0-alpha}"
HERE="$(cd "$(dirname "$0")" && pwd)"
SPEC="$REPO/src/sdk/addons/zz-external-context-kit.test.ts"
cat > "$SPEC" <<EOS
import { describe, expect, it } from "vitest";
import { readFileSync } from "node:fs";
import { validateAddOnManifest } from "./validation";
const manifest = JSON.parse(readFileSync("$HERE/addon.json", "utf8"));
describe("external addon.context-kit vs real validator", () => {
  it("validates with zero errors and zero warnings (sideload)", () => {
...
EOS
trap 'rm -f "$SPEC"' EXIT
cd "$REPO" && npx vitest run src/sdk/addons/zz-external-context-kit.test.ts
